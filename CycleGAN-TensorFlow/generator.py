...
class Generator:
  def __init__(self, name, is_training, ngf=64, norm='instance', image_height=240, image_width=320):
    self.name = name
    self.reuse = False
    self.ngf = ngf
    self.norm = norm
  ...
  def __call__(self, input):
    ...
    with tf.variable_scope(self.name):
      ...
      u32 = ops.uk(u64, self.ngf, is_training=self.is_training, norm=self.norm,
          reuse=self.reuse, name='u32', output_height=self.image_height, output_width=self.image_width)         # (?, w, h, 32)
